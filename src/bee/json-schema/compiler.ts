...
import { type IGeneralFieldState } from "../core";
import { untracked, hasCollected } from "@formily/reactive";
import { traverse, traverseSchema, isNoNeedCompileObject, hasOwnProperty, patchStatePageSchema } from "./shared";
...
export const patchSchemaCompile = (targetState: IGeneralFieldState, sourceSchema: ISchema, scope: any, demand = false) => {
  traverseSchema(sourceSchema, (value, path, omitCompile) => {
    ...
    if (demand) {
      if (collected || !targetState.initialized) {
        patchStatePageSchema(targetState, path, compiled);
      }
    } else {
      patchStatePageSchema(targetState, path, compiled);
    }
