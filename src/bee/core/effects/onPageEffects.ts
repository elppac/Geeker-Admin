...
function createPageEffect(type: LifeCycleTypes) {
  return createEffectHook(
    type,
    (page: Page) => (callback: (page: Page) => void) => {
      batch(() => {
        callback(page)
      })
    }
...
export const onPageValuesChange = createPageEffect(
  LifeCycleTypes.ON_FORM_VALUES_CHANGE
)
export const onPageInitialValuesChange = createPageEffect(
  LifeCycleTypes.ON_FORM_INITIAL_VALUES_CHANGE
)
export const onPageInputChange = createPageEffect(
  LifeCycleTypes.ON_FORM_INPUT_CHANGE
)
export const onPageGraphChange = createPageEffect(
