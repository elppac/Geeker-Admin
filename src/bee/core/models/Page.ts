...
import {
  ...
  type IModelGetter,
  type IModelSetter,
  type IFieldStateGetter,
  type IFieldStateSetter,
  type PageDisplayTypes,
  type IPageMergeStrategy,
  LifeCycleTypes
} from "../types";
import {
  createStateGetter,
  ...
  triggerPageInitialValuesChange,
  triggerPageValuesChange,
  setLoading,
  getValidPageValues
...
const DEV_TOOLS_HOOK = "__FORMILY_DEV_TOOLS_HOOK_X__";

export class Page<ValueType extends object = any> {
  displayName = "Page";
  id!: string;
  initialized!: boolean;
  validating!: boolean;
  ...
  constructor(props: IPageProps<ValueType>) {
  ...
  protected initialize(props: IPageProps<ValueType>) {
    this.id = uid();
    this.props = { ...props };
  ...
  protected makeValues() {
    this.values = getValidPageValues(this.props.values);
    this.initialValues = getValidPageValues(this.props.initialValues);
  }
  ...
  protected makeReactive() {
    this.disposers.push(
      observe(
        this,
        change => {
          triggerPageInitialValuesChange(this, change);
          triggerPageValuesChange(this, change);
        },
        true
      )
    );
  ...
  setValues = (values: any, strategy: IPageMergeStrategy = "merge") => {
  ...
  setInitialValues = (initialValues: any, strategy: IPageMergeStrategy = "merge") => {
  ...
  setDisplay = (display: PageDisplayTypes) => {
    this.display = display;
  };

  setPattern = (pattern: PagePatternTypes) => {
    this.pattern = pattern;
  };

  addEffects = (id: any, effects: IPageProps["effects"]) => {
  ...
  setEffects = (effects: IPageProps["effects"]) => {
  ...
  queryFeedbacks = (search: ISearchFeedback): IPageFeedback[] => {
  ...
  setState: IModelSetter<IPageState<ValueType>> = createStateSetter(this);

  getState: IModelGetter<IPageState<ValueType>> = createStateGetter(this);

  setPageState: IModelSetter<IPageState<ValueType>> = createStateSetter(this);

  getPageState: IModelGetter<IPageState<ValueType>> = createStateGetter(this);
  ...
  setPageGraph = (graph: IPageGraph) => {
    this.graph.setGraph(graph);
  };

  clearPageGraph = (pattern: FormPathPattern = "*", forceClear = true) => {
    this.query(pattern).forEach(field => {
