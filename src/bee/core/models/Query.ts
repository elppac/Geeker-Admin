...
export class Query {
  ...
  constructor(props: IQueryProps) {
    ...
    if (!this.pattern.isMatchPattern) {
      const matched = takeMatchPattern(
        this.page,
        this.pattern.haveRelativePattern ? buildDataPath(props.page.fields, this.pattern) : this.pattern
      );
      if (matched) {
  ...
  map(iterator?: any): any {
    return this.addresses.map(address => output(this.page.fields[address], iterator));
  }

  forEach<Result>(iterator: (field: GeneralField, address: FormPath) => Result) {
    return this.addresses.forEach(address => output(this.page.fields[address], iterator));
  }
  ...
  reduce<Result>(reducer: (value: Result, field: GeneralField, address: FormPath) => Result, initial?: Result): Result {
    return this.addresses.reduce(
      (value, address) => output(this.page.fields[address], (field, address) => reducer(value, field, address)),
      initial
    );
  }
