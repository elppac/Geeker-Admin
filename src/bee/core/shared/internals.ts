...
import { DataChange, autorun, batch, contains, isObservable, reaction, toJS, untracked } from "@formily/reactive";
import { ArrayField, Field, ObjectField, Page } from "../models";
import {
  LifeCycleTypes,
...
import { isArrayField, isDataField, isGeneralField, isObjectField, isPage, isQuery, isVoidField } from "./externals";

const hasOwnProperty = Object.prototype.hasOwnProperty;

const notify = (target: Page | Field, pageType: LifeCycleTypes, fieldType: LifeCycleTypes) => {
  if (isPage(target)) {
...
export const patchPageValues = (page: Page, path: Array<string | number>, source: any) => {
...
export const matchFeedback = (search?: ISearchFeedback, feedback?: IPageFeedback) => {
...
export const queryFeedbackMessages = (field: Field, search: ISearchFeedback) => {
  ...
  return queryFeedbacks(field, search).reduce((buf, info) => (isEmpty(info.messages) ? buf : buf.concat(info.messages)), []);
};
...
export const spliceArrayState = (field: ArrayField, props?: ISpliceArrayStateProps) => {
  ...
  const isDeleteNode = (identifier: string) => {
    ...
    const index = Number(number);
    return (
      (index > startIndex && !fields[`${preStr}${afterStr.replace(/^\.\d+/, `.${index + deleteCount}`)}`]) || index === startIndex
    );
  };
...
export const subscribeUpdate = (page: Page, pattern: FormPath, callback: (...args: any[]) => void) => {
  const updates = FormPath.ensureIn(page, "requests.updates", []);
  const indexes = FormPath.ensureIn(page, "requests.updateIndexes", {});
...
export const triggerPageInitialValuesChange = (page: Page, change: DataChange) => {
  if (Array.isArray(change.object) && change.key === "length") return;
  if (contains(page.initialValues, change.object) || page.initialValues === change.value) {
...
export const triggerPageValuesChange = (page: Page, change: DataChange) => {
  if (Array.isArray(change.object) && change.key === "length") return;
  if ((contains(page.values, change.object) || page.values === change.value) && page.initialized) {
