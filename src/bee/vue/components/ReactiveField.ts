...
import { inject, provide, type Ref, ref, shallowRef, watch } from "vue";
...
import { useField, usePage } from "../hooks";

import type { IReactiveFieldProps, VueComponentProps, DefineComponent } from "../types";
...
const resolveComponent = (render: () => unknown[], extra?: any) => {
  ...
  if (extra.length > 1 || extra?.render?.length > 1) {
    return (scopedProps: VueComponentProps<any>) => [...render(), h(extra, { props: scopedProps }, {})];
  }
  return () => [...render(), h(extra, {}, {})];
};

const mergeSlots = (field: GeneralField, slots: Record<string, any>, content: any): Record<string, (...args: any) => any[]> => {
  ...
  const patchSlot = (slotName: string) => () => slots[slotName]?.({ field, page: field.page }) ?? [];
...
export default observer({
  ...
  setup(props: IReactiveFieldProps, { slots }) {
    ...
    let createField = () =>
      pageRef?.value?.[`create${props.fieldType}`]?.({
        ...props.fieldProps,
    ...
    return () => {
      ...
      const renderDecorator = (childNodes: any[]) => {
        ...
        const finalComponent = FormPath.getIn(options?.components, field.decoratorType as string) ?? field.decoratorType;
      ...
      const renderComponent = () => {
        ...
        const component = FormPath.getIn(options?.components, field.componentType as string) ?? field.componentType;
